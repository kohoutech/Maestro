...
namespace Transonic.MIDI.Engine
{
    public class Transport
    {
        ...
        public Transport(IMidiView _window)
        {
            ...
            outDev = new OutputDevice(0, "output");
...
//- operation methods ---------------------------------------------------------
        ...
        public void setSequencePos(int ticknum)
        {
            ...
            {
                events = seq.tracks[track].events;
                trackPos[track] = 0;
        ...
        public void OnPulse(object sender, EventArgs e)
        {
            ...
            while (runningTime > tickTime)          //we've passed one or more ticks
            {
                ...
                for (int track = 1; track < trackCount; track++)
                {
                    ...
                    bool done = (trackPos[track] >= events.Count);
                    while (!done && tickCount >= events[trackPos[track]].time)
                    {
                        Message msg = events[trackPos[track]].msg;
                        outDev.sendShortMsg(msg);
                        window.handleMessage(track, msg);
                        trackPos[track]++;
                        done = (trackPos[track] >= events.Count);
                    }
